...
from .governance import (
    Decision, EvidenceKind, EvidenceProvenance, EvidenceRecordV2, GovernedMemoryFrame, TemporalValidity,
)
...
from .retrieval import BM25Retriever, DenseRetriever, FaissBinaryRetriever, ReferenceBinaryRetriever, RetrievalHit
...
class HNGMemory:
    ...
    def activate_profile(self, conversation_id: str, user_id: str,
        ...
        return self.profiles.activate(str(conversation_id), user_id, override)
    ...
    def ingest_evidence(self, *, content: str, semantics: SemanticState | Mapping[str, SemanticValue],
        ...
        record = EvidenceRecordV2(
            ...
            episode_id=str(episode_id), conversation_id=str(conversation_id), kind=EvidenceKind(kind),
    ...
    def _retrieve_ids(self, query: SemanticState, allowed: set[str], *, candidate_k: int,
        ...
        if lexical_query:
            for hit in self._lexical.search(lexical_query, top_k=candidate_k, allowed_ids=allowed):
                ...
                if previous is None or hit.score > previous.score:
                    hits[hit.evidence_id] = hit
        ranked_list = list(sorted(hits.values(), key=lambda item: (-item.score, item.evidence_id))[:candidate_k])
        seen = {hit.evidence_id for hit in ranked_list}
        ranked_list.extend(RetrievalHit(value, 0.0, "governance-priority") for value in priority_ids if value in allowed and value not in seen)
    ...
    def context(self, conversation_id: str, *, query: SemanticState | None = None,
        ...
        with self._lock:
            state, open_loops, constraints = self.store.working_state(str(conversation_id))
            query = query or state
            plan = self.planner.plan(intent)
            allowed, profile, prefiltered = self._access_ids(str(conversation_id), query=query)
            selected, hits = self._retrieve_ids(query, allowed, candidate_k=plan.candidate_k, lexical_query=lexical_query,
                                                priority_ids=self._priority_ids(str(conversation_id), query, limit=plan.candidate_k))
            ...
            return GovernedMemoryFrame(3, plan.intent.value, str(conversation_id), state, assessment,
                                       {} if profile is None else profile.as_dict(), open_loops, constraints, len(hits))
    ...
    def evaluate_action(self, state: SemanticState | Mapping[str, SemanticValue], action: SemanticValue, *,
        ...
        with self._lock:
            ...
            assessment = self.aggregator.assess(selected, query, plan, profile=profile)
            assessment = self._with_prefilter_exclusions(assessment, prefiltered, query)
            current, open_loops, constraints = self.store.working_state(str(conversation_id))
            return GovernedMemoryFrame(3, plan.intent.value, str(conversation_id), current, assessment,
                                       {} if profile is None else profile.as_dict(), open_loops, constraints, len(hits))
