...
def test_shared_sqlite_handle_diagnostic_v2_routes_exact_commit() -> None:
    command = reproduce.commands_for(argparse.Namespace(
        command="shared-sqlite-handle-diagnostic-v2",
        prepare_only=False,
        preregistered_commit="shared456",
    ))[0]
...
def test_shared_sqlite_handle_type_diagnostic_v2_routes_exact_commit() -> None:
    command = reproduce.commands_for(argparse.Namespace(
        command="shared-sqlite-handle-type-diagnostic-v2",
        prepare_only=False,
        preregistered_commit="typed-v2",
    ))[0]
...
def test_shared_sqlite_wal_index_diagnostic_routes_exact_commit() -> None:
    command = reproduce.commands_for(argparse.Namespace(
        command="shared-sqlite-wal-index-diagnostic",
        prepare_only=False,
        preregistered_commit="wal-index",
    ))[0]
