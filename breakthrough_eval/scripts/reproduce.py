...
def commands_for(args: argparse.Namespace) -> list[Command]:
    ...
    if args.command == "handle-observer-diagnostic-v3":
        argv = [
            sys.executable,
            str(SCRIPTS / "handle_observer_diagnostic_v3.py"),
        ]
        if args.prepare_only:
            argv.append("--prepare-only")
        else:
            if not args.preregistered_commit:
                raise ValueError(
                    "handle-observer-diagnostic-v3 execution requires "
                    "--preregistered-commit"
                )
            argv.extend([
                "--preregistered-commit", args.preregistered_commit,
        ...
        return [Command(
            "child_handle_observer_effect_diagnostic_v3",
            tuple(argv),
    ...
    if args.command == "shared-sqlite-handle-diagnostic-v2":
        argv = [
            sys.executable,
            str(SCRIPTS / "shared_sqlite_handle_diagnostic_v2.py"),
        ]
        if args.prepare_only:
            argv.append("--prepare-only")
        else:
            if not args.preregistered_commit:
                raise ValueError(
                    "shared-sqlite-handle-diagnostic-v2 execution requires "
                    "--preregistered-commit"
                )
            argv.extend([
                "--preregistered-commit", args.preregistered_commit,
        ...
        return [Command(
            "shared_sqlite_child_handle_root_cause_matrix_v2",
            tuple(argv),
            "Independent-sampler root-cause matrix; no qualification claim.",
        )]
    if args.command == "shared-sqlite-handle-type-diagnostic":
        argv = [
            sys.executable,
            str(SCRIPTS / "shared_sqlite_handle_type_diagnostic.py"),
        ]
        if args.prepare_only:
            argv.append("--prepare-only")
        else:
            if not args.preregistered_commit:
                raise ValueError(
                    "shared-sqlite-handle-type-diagnostic execution requires "
                    "--preregistered-commit"
                )
            argv.extend([
                "--preregistered-commit", args.preregistered_commit,
        ...
        return [Command(
            ...
            tuple(argv),
            "Windows handle-type mechanism diagnostic; no qualification claim.",
        )]
    if args.command == "shared-sqlite-handle-type-diagnostic-v2":
        argv = [
            sys.executable,
            str(SCRIPTS / "shared_sqlite_handle_type_diagnostic_v2.py"),
        ]
        if args.prepare_only:
            argv.append("--prepare-only")
        else:
            if not args.preregistered_commit:
                raise ValueError(
                    "shared-sqlite-handle-type-diagnostic-v2 execution "
        ...
        return [Command(
            "shared_sqlite_child_handle_type_diagnostic_v2",
            tuple(argv),
...
def parse_args() -> argparse.Namespace:
    ...
    handle_observer_v3 = subparsers.add_parser(
        "handle-observer-diagnostic-v3",
        help="Prepare or run the timing-robust attribution diagnostic.",
    )
    ...
    shared_handle_v2 = subparsers.add_parser(
        "shared-sqlite-handle-diagnostic-v2",
        help="Prepare or run the independent-sampler shared-SQLite matrix.",
    )
    shared_handle_v2.add_argument("--prepare-only", action="store_true")
    shared_handle_v2.add_argument("--preregistered-commit")
    shared_handle_type = subparsers.add_parser(
        "shared-sqlite-handle-type-diagnostic",
        help="Prepare or run the shared-SQLite Windows handle-type diagnostic.",
    )
    shared_handle_type.add_argument("--prepare-only", action="store_true")
    shared_handle_type.add_argument("--preregistered-commit")
    shared_handle_type_v2 = subparsers.add_parser(
        "shared-sqlite-handle-type-diagnostic-v2",
        help="Prepare or run the corrected Windows handle-type diagnostic.",
    )
