...
| 10 | Official LoCoMo/LoCoMo-Plus | PARTIAL | The official 2,387-sample input is pinned. The observed fixed 30-sample slice preserves a significant loss to full context. A separately preregistered, disjoint 30-sample 16/32/64-turn holdout completes 180/180 events: k64 reaches 13.5/30 versus k16 at 11/30 and full context at 14/30, but the primary CI includes zero. HNG, Strong, and BM25 exactly tie at fixed k64. The protocol remains noncanonical and contemporary-system comparison is absent. |
...
| 12 | Strong RAG plus HNG | PARTIAL | A public-data BM25 retrieval-budget sweep now exists, but realistic hybrid/dense/reranked retrieval is still missing; at fixed k64 HNG exactly ties BM25 and Strong. |
...
| 27 | Cost/token efficiency | PARTIAL | Fixed 27B arm prompt tokens and latency are recorded. On the disjoint public-data holdout, k64 uses 116,800 prompt tokens versus 680,824 for full context while scoring 13.5 versus 14.0; preprocessing/API cost and broader public workloads remain. |
...
| 34 | Statistical standard | PARTIAL | Paired bootstrap and exact McNemar cover both 30-sample public-data slices, including a preregistered disjoint retrieval-budget holdout; 20 independent-store repeats provide latency intervals. Multiple model seeds/families and canonical public units remain missing. |
...
| 45 | Most-important same-retrieval A/B | PARTIAL | Controlled frozen LLM study is complete on synthetic evidence; public-data fixed-candidate arms preserve identical retrieval and yield HNG/Strong/BM25 ties at both k16 development and k64 disjoint holdout points. Wider retrieval nearly reaches full context but does not reveal a governance gain. A real long-running task remains missing. |
...
| 2: public external validation | LongMemEval-V2, PersonaMem-v2, and expanded LoCoMo-Plus runs are noncanonical and lack contemporary-system comparisons. The 30-sample LoCoMo result is a statistically supported loss to full context and an exact tie with BM25/Strong. | UNMET |
