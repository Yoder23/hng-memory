...
| 29 | Long-run reliability soak | PARTIAL | Restart/fault/concurrency/20K-turn tests pass. A preregistered 1,000,000-write production-store probe passes 9/9 graceful restart checks, exact 100-tenant isolation, lifecycle mutations, and full-ledger backup/restore identity with independently matched database/backup hashes. This closes the million-write checkpoint only; repeated backup cycles, OS-crash, disk-full, long-run resource-growth, and hours-long concurrent soak evidence remain absent. |
