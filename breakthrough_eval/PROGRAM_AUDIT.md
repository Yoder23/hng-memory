...
| 29 | Long-run reliability soak | PARTIAL | Restart/fault/concurrency/20K-turn tests and the bounded million-write probe pass. The exact two-hour/12-process v1 attempt failed: its first online backup starved under sustained writes, no backup cycle completed, the 15-minute rotation was missed, WAL reached 10.24 GB, and handles reached 815 before a safety interrupt. Failure-driven v2 (write-quiesced/read-live backup with parent monitoring and timeout) is PREPARED_NOT_EXECUTED and supplies no behavioral evidence. Repeated qualifying backup cycles, OS-crash, disk-full, long-run resource-growth, and hours-long concurrent soak evidence remain absent. |
